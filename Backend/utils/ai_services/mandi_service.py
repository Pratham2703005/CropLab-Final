...
SYSTEM_PROMPT = (
    ...
    "multi-month wholesale-price monthly trend for the farmer's district (with "
    "the state-wide average for comparison) and (b) the farm's current "
    "crop-health signals from satellite imagery. Produce ONE short paragraph "
    "(max ~110 words) the farmer can read at a glance.\n\n"
    "The trend may cover fewer months than expected when the crop is "
    "out of season — describe only the months actually present in the JSON.\n\n"
    "Communicate, in roughly this order:\n"
    "1. The latest monthly modal price for the farmer's district and the "
    "direction of the trend across the months shown (up / down / flat).\n"
